...
public class MSAuthScreen extends Screen {
    ...
    private void auth(String query) {
        try {
            ...
            if (query.equals("error=access_denied&error_description=The user has denied access to the scope requested by the client application."))
                throw new AuthException(new TranslationTextComponent("ias.msauth.error.revoked"));
    ...
    public void error(Throwable t) {
        minecraft.execute(() -> {
            if (t instanceof AuthException) {
                error = font.split(((AuthException) t).getComponent().getColoredString(), width - 20);
            } else {
                error = font.split(I18n.get("ias.auth.unknown", t.toString()), width - 20);
            }
        });
    }
    ...
    @Override
    public void init() {
        addButton(new Button(this.width / 2 - 75, this.height - 28, 150, 20, I18n.get("gui.cancel"), btn -> minecraft.setScreen(prev)));
    }
    ...
    public void render(int mouseX, int mouseY, float delta) {
        ...
        if (error != null) {
            for (int i = 0; i < error.size(); i++) {
                drawCenteredString(font, error.get(i), this.width / 2, height / 2 - 5 + i * 10 - error.size() * 5, 0xFFFF0000);
                if (i > 6) break; //Exceptions can be very large.
            }
        } else {
            drawCenteredString(font, state, width / 2, height / 2 - 10, -1);
            drawCenteredString(font, symbols[tick % symbols.length], width / 2, height / 2, 0xFFFF9900);
        }
