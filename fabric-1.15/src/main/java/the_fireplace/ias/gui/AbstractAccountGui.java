...
public class AbstractAccountGui extends Screen {
    ...
    public void init() {
        ...
        username = addButton(new EditBox(font, this.width / 2 - 100, 60, 200, 20, I18n.get("ias.username")));
    ...
    public void tick() {
        ...
        complete.setMessage(!username.getValue().trim().isEmpty() && password.getValue().isEmpty() ? this.title.copy().append(" ").append(new TranslatableComponent("ias.offline")).getColoredString() : this.title.getColoredString());
    ...
    public void end() {
        if (password.getValue().isEmpty()) {
            ...
            if (Config.accounts.stream().anyMatch(acc -> acc.alias().equalsIgnoreCase(name))) {
                error = font.split(I18n.get("ias.auth.alreadyexists"), width - 10);
                return;
            }
            logging = true;
