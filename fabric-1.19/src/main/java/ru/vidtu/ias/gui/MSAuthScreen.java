...
public class MSAuthScreen extends Screen {
    ...
    public MSAuthScreen(Screen prev, Consumer<Account> handler) {
        super(Component.translatable("ias.msauth.title"));
        this.prev = prev;
    ...
    private void auth(String query) {
        try {
            ...
            if (query.equals("error=access_denied&error_description=The user has denied access to the scope requested by the client application."))
                throw new AuthException(Component.translatable("ias.msauth.error.revoked"));
            ...
            if (Config.accounts.stream().anyMatch(acc -> acc.alias().equalsIgnoreCase(profile.getRight())))
                throw new AuthException(Component.translatable("ias.auth.alreadyexists"));
            minecraft.execute(() -> {
    ...
    public void error(Throwable t) {
        minecraft.execute(() -> {
            if (t instanceof AuthException) {
                ...
                error = font.split(Component.translatable("ias.auth.unknown", t.toString()), width - 20);
            }
        });
    }
    ...
    @Override
    public void init() {
        addRenderableWidget(new Button(this.width / 2 - 75, this.height - 28, 150, 20, Component.translatable("gui.cancel"), btn -> minecraft.setScreen(prev)));
    }
