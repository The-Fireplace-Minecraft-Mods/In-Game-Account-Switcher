...
public class SkinRenderer {
	 ...
	 */
	public static void loadAllAsync(MinecraftClient mc, boolean forceRecache, Runnable onEnd) {
		Overlay oldov = mc.getOverlay();
		...
		new Thread(() -> {
			...
			slo.progress = 1F;
			mc.execute(() -> mc.setOverlay(oldov));
			mc.execute(onEnd);
	 ...
	 */
	public static void loadSkin(MinecraftClient mc, String name, UUID uuid, boolean forceRecache) {
		loadSkin(mc, name, uuid, new File(new File(mc.runDirectory, "cachedImages/models"), name + ".png"),
				new File(new File(mc.runDirectory, "cachedImages/faces"), name + ".png"), forceRecache);
	}
	 ...
	 * @param forceRecache Should we force reload existing model and face
	 */
	public static void loadSkin(MinecraftClient mc, String name, UUID uuid, File modelF, File faceF, boolean forceRecache) {
	...
	public static class SkinsLoadingOverlay extends Overlay {
		private static final Identifier PROGRESS = new Identifier("ias", "textures/iaslogo.png");
		...
		public void render(MatrixStack ms, int mx, int my, float delta) {
		    ...
		    Screen.fill(ms, 0, 0, w, h, 0xFF101010);
		    RenderSystem.setShaderTexture(0, PROGRESS);
		    Screen.drawTexture(ms, w / 2 - 25, h / 2 - 25, 0F, 0F, 50, 50, 50, 50);
		    Screen.fill(ms, 30, h / 4 * 3, w - 30, h / 4 * 3 + 10, -1);
		    Screen.fill(ms, 31, h / 4 * 3 + 1, w - 31, h / 4 * 3 + 9, -16777216);
		    Screen.fill(ms, 31, h / 4 * 3 + 1, (int) (31 + (w - 61) * smoothProgress), h / 4 * 3 + 9, 0xFF00AA00);
		}
	}
}
