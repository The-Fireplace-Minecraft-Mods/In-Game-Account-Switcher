...
public final class AccountScreen extends Screen implements LastPassRenderCallback {
    ...
    protected void init() {
        ...
        this.login = new Button(this.width / 2 - 50 - 100 - 4, this.height - 24 - 24, 100, 20, Component.translatable("ias.accounts.login"), btn -> this.list.login(true, IASConfig.closeOnLogin ? () -> this.minecraft.setScreen(this.parent) : null), this.loginTooltip);
        this.addRenderableWidget(this.login);

        // Add offline login button.
        this.offlineLogin = new Button(this.width / 2 - 50 - 100 - 4, this.height - 24, 100, 20, Component.translatable("ias.accounts.offlineLogin"), btn -> this.list.login(false, IASConfig.closeOnLogin ? () -> this.minecraft.setScreen(this.parent) : null));
