 ...
 * @author VidTu
 */
@Mod(modid = "ias", name = "In-Game Account Switcher", version = "D7.29 :P", useMetadata = true,
    ...
    public void onScreenRender(GuiScreenEvent.DrawScreenEvent.Post event) {
        if (event.gui instanceof GuiMainMenu && Config.titleScreenText) {
            if (Config.titleScreenTextAlignment == Config.Alignment.LEFT) {
                event.gui.drawString(event.gui.mc.fontRenderer, I18n.format("ias.title", event.gui.mc.getSession().getUsername()), tx, ty, 0xFFCC8888);
            } else if (Config.titleScreenTextAlignment == Config.Alignment.RIGHT) {
                event.gui.drawString(event.gui.mc.fontRenderer, I18n.format("ias.title", event.gui.mc.getSession().getUsername()), tx - event.gui.mc.fontRenderer.getStringWidth(I18n.format("ias.title", event.gui.mc.getSession().getUsername())), ty, 0xFFCC8888);
            } else {
                event.gui.drawCenteredString(event.gui.mc.fontRenderer, I18n.format("ias.title", event.gui.mc.getSession().getUsername()), tx, ty, 0xFFCC8888);
            }
        }
        if (button != null && event.gui.buttonList.contains(button) && button.func_146115_a()) { // button.isHovered()
