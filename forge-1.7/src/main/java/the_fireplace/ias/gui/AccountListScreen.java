...
public class AccountListScreen extends GuiScreen {
    ...
    public void initGui() {
        ...
        buttonList.add(reloadSkins = new GuiButton(0, 2, 2, 120, 20, I18n.format("ias.listGui.reloadSkins")));
        search = new GuiTextField(this.fontRendererObj, this.width / 2 - 80, 14, 160, 16);
        buttonList.add(add = new GuiButton(2, this.width / 2 + 4 + 40, this.height - 52, 120, 20, I18n.format("ias.listGui.add")));
        buttonList.add(login = new GuiButton(3, this.width / 2 - 154 - 10, this.height - 52, 120, 20, I18n.format("ias.listGui.login")));
        buttonList.add(loginOffline = new GuiButton(4, this.width / 2 - 154 - 10, this.height - 28, 110, 20, I18n.format("ias.listGui.loginOffline")));
        buttonList.add(edit = new GuiButton(5, this.width / 2 - 40, this.height - 52, 80, 20, I18n.format("ias.listGui.edit")));
        buttonList.add(delete = new GuiButton(6, this.width / 2 - 50, this.height - 28, 100, 20, I18n.format("ias.listGui.delete")));
        buttonList.add(new GuiButton(7, this.width / 2 + 4 + 50, this.height - 28, 110, 20, I18n.format("gui.cancel")));
        updateButtons();
    ...
    public void mouseClicked(int mx, int my, int btn) {
        if (list.func_148179_a(mx, my, btn)) return; // list.mouseClicked()
    ...
    public void updateScreen() {
        if (!prevSearch.equals(search.getText())) {
            ...
            list.updateAccounts(prevSearch);
        }
    ...
    public void drawScreen(int mx, int my, float delta) {
        ...
        search.drawTextBox();
        if (search.getText().isEmpty()) drawString(fontRendererObj, I18n.format("ias.listGui.search"), search.xPosition + 4, search.yPosition + (search.height - 8) / 2, 0xFF808080);
        super.drawScreen(mx, my, delta);
        drawCenteredString(fontRendererObj, "In-Game Account Switcher", this.width / 2, 4, -1);
        if (list.selectedElement() >= 0) {
            ...
            GL11.glPushMatrix();
            GL11.glScalef(4, 4, 4);
            GL11.glTranslated(1, height / 8D - 16D - 4D, 0);
            // FIXME:
//            GuiScreen.func_146110_a(4, 0, 8, 8, 8, 8, 64, 64); // Head -> GuiScreen.drawModalRectWithCustomSizedTexture
//            GuiScreen.func_146110_a(4, 8, 20, 20, 8, 12, 64, 64); // Body -> GuiScreen.drawModalRectWithCustomSizedTexture
//            GuiScreen.func_146110_a(slim ? 1 : 0, 8, 44, 20, slim ? 3 : 4, 12, 64, 64); // Right Arm (Left from our perspective) -> GuiScreen.drawModalRectWithCustomSizedTexture
//            GuiScreen.func_146110_a(12, 8, 36, 52, slim ? 3 : 4, 12, 64, 64); // Left Arm (Right from our perspective) -> GuiScreen.drawModalRectWithCustomSizedTexture
//            GuiScreen.func_146110_a(4, 20, 4, 20, 4, 12, 64, 64); // Right Leg (Left from our perspective) -> GuiScreen.drawModalRectWithCustomSizedTexture
//            GuiScreen.func_146110_a(8, 20, 20, 52, 4, 12, 64, 64); // Left Leg (Right from our perspective) -> GuiScreen.drawModalRectWithCustomSizedTexture
//            GuiScreen.func_146110_a(4, 0, 40, 8, 8, 8, 64, 64); // Head (Overlay) -> GuiScreen.drawModalRectWithCustomSizedTexture
//            GuiScreen.func_146110_a(slim ? 1 : 0, 8, 44, 36, slim ? 3 : 4, 12, 64, 64); // Right Arm (Overlay) -> GuiScreen.drawModalRectWithCustomSizedTexture
//            GuiScreen.func_146110_a(12, 8, 52, 52, slim ? 3 : 4, 12, 64, 64); // Left Arm (Overlay) -> GuiScreen.drawModalRectWithCustomSizedTexture
//            GuiScreen.func_146110_a(4, 20, 4, 36, 4, 12, 64, 64); // Right Leg (Overlay) -> GuiScreen.drawModalRectWithCustomSizedTexture
//            GuiScreen.func_146110_a(8, 20, 4, 52, 4, 12, 64, 64); // Left Leg (Overlay) -> GuiScreen.drawModalRectWithCustomSizedTexture
            GL11.glPopMatrix();
        }
        if (state != null) {
            drawCenteredString(fontRendererObj, state, this.width / 2, this.height - 62, 0xFFFF9900);
        }
    }
    ...
    private void login() {
        ...
        acc.login((s, o) -> state = I18n.format(s, o)).whenComplete((d, t) -> {
            state = null;
            if (t != null) {
                mc.func_152344_a(() -> mc.displayGuiScreen(new IASAlertScreen(() -> mc.displayGuiScreen(this),
