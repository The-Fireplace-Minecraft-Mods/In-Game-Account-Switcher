...
public class MSAuthScreen extends Screen {
    ...
    public void error(Throwable t) {
        minecraft.execute(() -> {
            if (t instanceof AuthException) {
                ...
                error = font.split(new TranslationTextComponent("ias.auth.unknown", t.toString()), width - 20);
            }
        });
    }
    ...
    @Override
    public void init() {
        addButton(new Button(this.width / 2 - 75, this.height - 28, 150, 20, new TranslationTextComponent("gui.cancel"), btn -> minecraft.setScreen(prev)));
    }
