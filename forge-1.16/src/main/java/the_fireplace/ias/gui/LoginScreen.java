...
public class LoginScreen extends Screen {
    ...
    private final ITextComponent buttonTip;
    ...
    public LoginScreen(Screen prev, ITextComponent title, ITextComponent buttonText, ITextComponent buttonTip, Consumer<Account> handler) {
        super(title);
        this.prev = prev;
    ...
    public void init() {
        super.init();
        addButton(offline = new Button(width / 2 - 152, this.height - 28, 150, 20, buttonText, btn -> loginOffline(), (btn, ms, mx, my) -> renderTooltip(ms, font.split(buttonTip, 150), mx, my)));
        offline.active = false;
        addButton(new Button(this.width / 2 + 2, this.height - 28, 150, 20, DialogTexts.GUI_CANCEL, btn -> minecraft.setScreen(prev)));
        username = addButton(new TextFieldWidget(font, this.width / 2 - 100, height / 2 - 12, 200, 20, username, new TranslationTextComponent("ias.loginGui.nickname")));
        username.setMaxLength(16);
        addButton(microsoft = new Button(this.width / 2 - 50, this.height / 2 + 12, 100, 20, new TranslationTextComponent("ias.loginGui.microsoft"), btn -> loginMicrosoft()));
    }
