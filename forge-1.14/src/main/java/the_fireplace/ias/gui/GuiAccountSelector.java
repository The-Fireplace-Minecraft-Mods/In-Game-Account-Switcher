...
public class GuiAccountSelector extends Screen {
    ...
    protected void init() {
        ...
        addButton(reloadskins = new Button(2, 2, 120, 20, I18n.get("ias.reloadskins"), btn -> reloadSkins()));
        addButton(new Button(this.width / 2 + 4 + 40, this.height - 52, 120, 20, I18n.get("ias.addaccount"), btn -> add()));
        addButton(login = new Button(this.width / 2 - 154 - 10, this.height - 52, 120, 20, I18n.get("ias.login"), btn -> accountsgui.login()));
        addButton(edit = new Button(this.width / 2 - 40, this.height - 52, 80, 20, I18n.get("ias.edit"), btn -> accountsgui.edit()));
        addButton(loginoffline = new Button(this.width / 2 - 154 - 10, this.height - 28, 110, 20, new TranslationTextComponent("ias.login").append(" ").append(new TranslationTextComponent("ias.offline")).getColoredString(), btn -> accountsgui.loginOffline()));
        addButton(new Button(this.width / 2 + 4 + 50, this.height - 28, 110, 20, I18n.get("gui.cancel"), btn -> minecraft.setScreen(prev)));
        addButton(delete = new Button(this.width / 2 - 50, this.height - 28, 100, 20, I18n.get("ias.delete"), btn -> accountsgui.delete()));
        addButton(search = new TextFieldWidget(this.font, this.width / 2 - 80, 14, 160, 16, I18n.get("ias.search")));
        updateButtons();
    ...
    public void render(int mx, int my, float delta) {
        ...
        if (accountsgui.getSelected() != null) {
            ...
            if (acc.online()) drawString(font, I18n.get("ias.premium"), width - 8 - 61, height / 2 - 64 - 13, 6618980);
            else drawString(font, I18n.get("ias.notpremium"), width - 8 - 61, height / 2 - 64 - 13, 16737380);
            drawString(font, I18n.get("ias.timesused"), width - 8 - 61, height / 2 - 64 - 15 + 12, -1);
            drawString(font, String.valueOf(acc.uses()), width - 8 - 61, height / 2 - 64 - 15 + 21, -1);
            if (acc.uses() > 0) {
                drawString(font, I18n.get("ias.lastused"), width - 8 - 61, height / 2 - 64 - 15 + 30, -1);
        ...
        public void login() {
            ...
            acc.login(minecraft, t -> {
                ...
                if (t == null) {
                    ...
                    error = ((AuthException) t).getComponent().getColoredString();
                } else {
                    IAS.LOG.warn("Unable to login", t);
                    error = I18n.get("ias.auth.unknown", t.toString());
                }
            });
        }
