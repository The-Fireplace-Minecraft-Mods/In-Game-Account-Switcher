...
public class MojangAccount implements Account {
    ...
    public void login(Minecraft mc, Consumer<Throwable> handler) {
        IAS.EXECUTOR.execute(() -> {
            ...
            mc.execute(() -> {
                mc.user = new Session(username, UUIDTypeAdapter.fromUUID(uuid), accessToken, "mojang");
                handler.accept(null);
            });
        });
    }
    ...
    public void refresh() throws Exception {
        ...
        if (r.response() < 200 || r.response() >= 300)
            throw new AuthException(new TranslationTextComponent("ias.auth.unknown", r.error()));
