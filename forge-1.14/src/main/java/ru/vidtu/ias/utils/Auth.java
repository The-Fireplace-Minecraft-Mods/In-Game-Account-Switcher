...
public class Auth {
    ...
    public static Pair<String, String> authXSTS(String xblToken) throws AuthException, IllegalArgumentException, Exception {
        ...
        if (pr.response() == 401) throw new AuthException(new TranslationTextComponent("ias.msauth.error.noxbox"));
    ...
    public static void checkGameOwnership(String accessToken) throws AuthException, IllegalArgumentException, Exception {
        ...
        if (IAS.GSON.fromJson(pr.body(), JsonObject.class).getAsJsonArray("items").size() == 0) throw new AuthException(new TranslationTextComponent("ias.msauth.error.gamenotowned"));
    }
    ...
    public static MojangAccount authMojang(String name, String pwd) throws AuthException, IOException {
		...
		if (r.response() < 200 || r.response() >= 300) {
			...
			try {
				...
				if (reqerr.toLowerCase().contains("cloudfront")) {
					throw new AuthException(new TranslationTextComponent("ias.mojauth.toofast"), reqerr); //CloudFront DoS/DDoS protection.
				}
				throw new AuthException(new TranslationTextComponent("ias.auth.unknown", reqerr), reqerr);
			}
			...
			if (err.equals("ForbiddenOperationException")) {
				...
				if (msg.equals("Invalid credentials. Invalid username or password.")) {
					throw new AuthException(new TranslationTextComponent("ias.mojauth.invalidcreds"), jo.toString());
				}
				if (msg.equals("Invalid credentials.")) {
					throw new AuthException(new TranslationTextComponent("ias.mojauth.toofast"), jo.toString());
				}
			}
			if (err.equals("ResourceException")) {
				throw new AuthException(new TranslationTextComponent("ias.mojauth.migrated"), jo.toString());
			}
			throw new AuthException(new TranslationTextComponent("ias.auth.unknown", jo.toString()));
		}
		...
		if (!respClientToken.equals(clientToken))
			throw new AuthException(new TranslationTextComponent("ias.auth.unknown",
