        ...
        ScreenEvents.AFTER_INIT.register((mc, screen, w, h) -> {
            if (screen instanceof JoinMultiplayerScreen && Config.multiplayerScreenButton) {
                ...
                Screens.getButtons(screen).add(new ImageButton(bx, by, 20, 20, 0, 0, 20, IAS_BUTTON,
                        ...
                        -> screen.renderTooltip(ms, new TextComponent("In-Game Account Switcher"), mx, my),
                    ...
                    try {
                        ...
                        ScreenEvents.afterRender(screen).register((s, ms, mx, my, delta) -> {
                            if (Config.titleScreenTextAlignment == Config.Alignment.LEFT) {
                                Gui.drawString(ms, mc.font, new TranslatableComponent("ias.title", mc.getUser().getName()), tx, ty, 0xFFCC8888);
                                return;
                            }
                            if (Config.titleScreenTextAlignment == Config.Alignment.RIGHT) {
                                Gui.drawString(ms, mc.font, new TranslatableComponent("ias.title", mc.getUser().getName()), tx - mc.font.width(new TranslatableComponent("ias.title", mc.getUser().getName())), ty, 0xFFCC8888);
                                return;
                            }
                            Gui.drawCenteredString(ms, mc.font, new TranslatableComponent("ias.title", mc.getUser().getName()), tx, ty, 0xFFCC8888);
                        });
                    } catch (Throwable t) {
                        int tx = w / 2;
