...
public class AccountListScreen extends Screen {
    ...
    public void init() {
        ...
        addWidget(list);
        addButton(reloadSkins = new Button(2, 2, 120, 20, new TranslatableComponent("ias.listGui.reloadSkins"), btn -> reloadSkins()));
        addButton(search = new EditBox(this.font, this.width / 2 - 80, 14, 160, 16, search, new TranslatableComponent("ias.listGui.search")));
        addButton(add = new Button(this.width / 2 + 4 + 40, this.height - 52, 120, 20, new TranslatableComponent("ias.listGui.add"), btn -> add()));
        addButton(login = new Button(this.width / 2 - 154 - 10, this.height - 52, 120, 20, new TranslatableComponent("ias.listGui.login"), btn -> login()));
        addButton(loginOffline = new Button(this.width / 2 - 154 - 10, this.height - 28, 110, 20, new TranslatableComponent("ias.listGui.loginOffline"), btn -> loginOffline()));
        addButton(edit = new Button(this.width / 2 - 40, this.height - 52, 80, 20, new TranslatableComponent("ias.listGui.edit"), btn -> edit()));
        addButton(delete = new Button(this.width / 2 - 50, this.height - 28, 100, 20, new TranslatableComponent("ias.listGui.delete"), btn -> delete()));
        addButton(new Button(this.width / 2 + 4 + 50, this.height - 28, 110, 20, CommonComponents.GUI_CANCEL, btn -> minecraft.setScreen(prev)));
        updateButtons();
    ...
    private void login() {
        ...
        acc.login((s, o) -> state = I18n.get(s, o)).whenComplete((d, t) -> {
            ...
            minecraft.execute(() -> {
                ...
                SocialInteractionsService socials = ((MinecraftAccessor) minecraft).ias$createSocialInteractions(new YggdrasilAuthenticationService(Proxy.NO_PROXY), new GameConfig(new GameConfig.UserData(minecraft.getUser(), null, null, null), null, null, null, null));
                ((MinecraftAccessor) minecraft).ias$socialInteractionsService(socials);
                ((MinecraftAccessor) minecraft).ias$playerSocialManager(new PlayerSocialManager(minecraft, socials));
            });
        });
    }
