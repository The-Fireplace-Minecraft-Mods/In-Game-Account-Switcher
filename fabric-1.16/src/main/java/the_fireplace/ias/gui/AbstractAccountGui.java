...
public class AbstractAccountGui extends Screen {
    ...
    public void init() {
        ...
        addButton(new Button(this.width / 2 + 2, this.height - 28, 150, 20, new TranslatableComponent("gui.cancel"), btn -> minecraft.setScreen(prev)));
        username = addButton(new EditBox(font, this.width / 2 - 100, 60, 200, 20, new TranslatableComponent("ias.username")));
        username.setMaxLength(512);
        password = addButton(new GuiPasswordField(font, this.width / 2 - 100, 90, 200, 20, new TranslatableComponent("ias.password")));
        password.setMaxLength(512);
        complete.active = false;
        addButton(new Button(this.width / 2 - 50, this.height / 3 * 2, 100, 20, new TranslatableComponent("ias.msauth.btn"), btn -> minecraft.setScreen(new MSAuthScreen(prev, handler))));
    }
