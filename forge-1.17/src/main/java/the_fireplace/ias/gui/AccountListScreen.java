...
public class AccountListScreen extends Screen {
    ...
    private void login() {
        ...
        acc.login((s, o) -> state = I18n.get(s, o)).whenComplete((d, t) -> {
            ...
            minecraft.execute(() -> {
                minecraft.user = new User(d.name(), UUIDTypeAdapter.fromUUID(d.uuid()), d.accessToken(), d.userType());
                SocialInteractionsService socials = minecraft.createSocialInteractions(new YggdrasilAuthenticationService(Proxy.NO_PROXY), new GameConfig(new GameConfig.UserData(minecraft.getUser(), null, null, null), null, null, null, null));
                minecraft.socialInteractionsService = socials;
                minecraft.playerSocialManager = new PlayerSocialManager(minecraft, socials);
            });
        });
    }
