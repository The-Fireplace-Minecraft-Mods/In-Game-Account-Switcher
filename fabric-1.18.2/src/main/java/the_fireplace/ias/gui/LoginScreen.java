...
public class LoginScreen extends Screen {
    ...
    public void init() {
        ...
        username = addRenderableWidget(new EditBox(font, this.width / 2 - 100, height / 2 - 12, 200, 20, username, new TranslatableComponent("ias.loginGui.nickname")));
        username.setMaxLength(16);
        addRenderableWidget(microsoft = new Button(this.width / 2 - 50, this.height / 2 + 12, 100, 20, new TranslatableComponent("ias.loginGui.microsoft"), btn -> loginMicrosoft()));
    }
    ...
    public void render(PoseStack ms, int mx, int my, float delta) {
        ...
        if (state != null) {
            ...
            drawCenteredString(ms, font, IAS.LOADING[(int) ((System.currentTimeMillis() / 50) % IAS.LOADING.length)], width / 2, height / 3 * 2 + 10, 0xFFFF9900);
        }
